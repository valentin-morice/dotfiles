...
`bin`), pins the repo-local git signing identity, scaffolds the IMAP mail
...
```sh
stow -d common  -t ~ alacritty bash claude git imap lazydocker lazygit systemd theme tmux xdg zsh
...
- **Mail backend** needs `~/.config/imap/imap.env` (gitignored, in the
  ...
  ```
  The daemon writes `/tmp/imap-$USER.txt`; the waybar `custom/mail` module (`waybar-mail`, in the `waybar` package) just reads it — already in place, no extra step.
