...
`tmux` follows the theme the same way: `theme-render` renders the palette into `~/.config/tmux/theme.conf`, which only sets `@theme_*` user options (accent, green, secondary, muted + the pill text colour); `.tmux.conf` sources it and its status/border styles reference them as `#{@theme_*}`, expanded at draw time. A theme switch re-sources that one file into the running server, so every attached client recolours live. (Named ANSI colours weren't enough: `black` stays near-black on both palettes by design, so black-on-accent pills lost contrast on light.)
